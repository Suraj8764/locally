...
router.get('/nearby', async (req, res) => {
    ...
    let sourceWorkers = mockData_1.workers;
    if (env_1.ENV.USE_DB) {
        const query = {};
        if (categoryId)
        ...
        if (onlyOnline)
            query.isOnline = true;
        sourceWorkers = await Worker_1.Worker.find(query).lean();
    }
...
router.get('/:id', async (req, res) => {
    let worker;
    if (env_1.ENV.USE_DB) {
        worker = await Worker_1.Worker.findById(req.params.id);
    }
    else {
        worker = mockData_1.workers.find((w) => w.id === req.params.id);
    }
    if (!worker) {
